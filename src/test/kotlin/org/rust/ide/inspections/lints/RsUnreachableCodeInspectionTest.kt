...
class RsUnreachableCodeInspectionTest : RsInspectionsTestBase(RsUnreachableCodeInspection::class) {
    fun `test straightline with return`() = checkByText("""
        fn main() {
            let x = 1;
            return;<warning descr="Unreachable code">
            let y = 2;
            let z = 3;</warning>
        }
    """)
    ...
    fun `test unreachable tail expr`() = checkByText("""
        fn foo() -> i32 {
            return 42;<warning descr="Unreachable code">
            123</warning>
        }
    """)
    ...
    fun `test code after loop`() = checkByText("""
        fn main() {
            loop {
                1;
            }<warning descr="Unreachable code">
            2;</warning>
        }
    """)
    ...
    fun `test unreachable in closure call`() = checkByText("""
        ...
        fn main() {
            foo(|x| {
                if x > 0 {
                    panic!();<warning descr="Unreachable code">
                    1;</warning>
                }
            });
            2;
    ...
    @MockAdditionalCfgOptions("intellij_rust")
    fun `test ignore conditionally enabled unreachable code`() = checkByText("""
        fn foo() -> i32 {
            return 1;
            #[cfg(intellij_rust)]
            return 2;
